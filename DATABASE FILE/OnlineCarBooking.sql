...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 22, 2025 at 01:16 AM
...
(1, 'C 180 Avantgarde', '123456', 4, '', 'Sedan', 1, 'vendor/img/vehicles/veh_1758021325_7528.webp', NULL, NULL, 'Available', 2);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_booking_grid`  AS SELECT `b`.`id` AS `booking_id`, `b`.`scheduled_start_at` AS `scheduled_at`, `b`.`created_at` AS `created_at`, coalesce(`b`.`contact_name`,'') AS `client_name`, `b`.`pax` AS `pax`, `b`.`pickup_point` AS `pickup`, `b`.`dropoff_point` AS `dropoff`, `tv`.`v_reg_no` AS `vehicle_reg_no`, `b`.`booking_type` AS `booking_type`, (select `a`.`name` from `accounts` `a` where `a`.`id` = `b`.`driver_id`) AS `driver_name`, `b`.`status` AS `status`, `b`.`driver_id` AS `driver_id` FROM (`bookings` `b` left join `tms_vehicle` `tv` on(`tv`.`v_id` = `b`.`vehicle_id`))union all select `u`.`u_id` AS `booking_id`,str_to_date(concat(nullif(`u`.`u_car_date`,''),' ',nullif(`u`.`u_car_time`,'')),'%Y-%m-%d %H:%i:%s') AS `scheduled_at`,case when `u`.`u_car_createdat` regexp '^[0-9]+$' then from_unixtime(`u`.`u_car_createdat`) else NULL end AS `created_at`,trim(concat(coalesce(`u`.`u_fname`,''),' ',coalesce(`u`.`u_lname`,''))) AS `client_name`,nullif(`u`.`u_car_pax`,'') + 0 AS `pax`,`u`.`u_car_pickup` AS `pickup`,`u`.`u_car_destination` AS `dropoff`,`u`.`u_car_regno` AS `vehicle_reg_no`,'admin' AS `booking_type`,`u`.`u_car_driver` AS `driver_name`,case when lcase(`u`.`u_car_book_status`) like '%approved%' then 'accepted' when lcase(`u`.`u_car_book_status`) regexp 'on trip|in service' then 'in_progress' when lcase(`u`.`u_car_book_status`) like '%cancel%' then 'cancelled' when lcase(`u`.`u_car_book_status`) like '%complete%' then 'completed' else 'pending' end AS `status`,NULL AS `driver_id` from `tms_user` `u` where `u`.`u_category` <> 'Driver'  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_fleet_summary`  AS SELECT (select count(0) from `tms_vehicle` `v` where `v`.`deleted_at` is null) AS `total_vehicles`, (select count(0) from `tms_vehicle` `v` where `v`.`deleted_at` is null and lcase(`v`.`v_status`) like 'avail%') AS `vehicles_available`, (select count(0) from `tms_vehicle` `v` where `v`.`deleted_at` is null and lcase(`v`.`v_status`) regexp 'book|service') AS `vehicles_in_use`, (select count(0) from `tms_vehicle` `v` where `v`.`deleted_at` is null and lcase(`v`.`v_status`) regexp 'maint') AS `vehicles_maintenance`, (select count(0) from `tms_vehicle` `v` where `v`.`deleted_at` is null and lcase(`v`.`v_status`) like 'inactive%') AS `vehicles_inactive`, (select count(0) from `tms_bookings` `b` where cast(`b`.`scheduled_at` as date) = curdate()) AS `trips_today`, (select count(0) from `tms_bookings` `b` where `b`.`status` = 'accepted') AS `trips_in_progress`, (select count(distinct `b`.`driver_id`) from `tms_bookings` `b` where cast(`b`.`scheduled_at` as date) = curdate() and `b`.`status` in ('accepted','completed')) AS `drivers_active_today` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_trips_in_progress_live`  AS SELECT count(0) AS `trips_in_progress_live`, count(distinct `booking_runs`.`driver_id`) AS `drivers_active_live` FROM `booking_runs` WHERE `booking_runs`.`pickup_button_at` is not null AND `booking_runs`.`dropoff_button_at` is null AND cast(coalesce(`booking_runs`.`pickup_button_at`,current_timestamp()) as date) = curdate() ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_vehicle_daily_metrics`  AS SELECT `br`.`vehicle_id` AS `vehicle_id`, cast(coalesce(`br`.`pickup_button_at`,`br`.`dropoff_button_at`) as date) AS `service_date`, count(distinct `br`.`booking_id`) AS `trips`, sum(`br`.`distance_km`) AS `distance_km`, sum(`br`.`fuel_used_liters`) AS `fuel_used_liters`, min(`br`.`odo_start_km`) AS `odo_start_km`, max(`br`.`odo_end_km`) AS `odo_end_km`, sum(`br`.`duration_seconds`) AS `duration_seconds` FROM `booking_runs` AS `br` GROUP BY `br`.`vehicle_id`, cast(coalesce(`br`.`pickup_button_at`,`br`.`dropoff_button_at`) as date) ;
...
ALTER TABLE `bookings`
  ...
  ADD KEY `idx_bookings_date_status` (`scheduled_start_at`,`status`);
...
ALTER TABLE `booking_runs`
  ...
  ADD KEY `idx_runs_vehicle_day` (`vehicle_id`,`pickup_button_at`,`dropoff_button_at`);
...
ALTER TABLE `telemetry_alerts`
  ...
  ADD KEY `idx_ta_vehicle_day` (`vehicle_id`,`created_at`);
...
ALTER TABLE `telemetry_samples`
  ...
  ADD KEY `idx_ts_vehicle_day` (`vehicle_id`,`recorded_at`);
...
ALTER TABLE `tms_bookings`
  ...
  ADD KEY `fk_tb_vehicle` (`vehicle_id`);
...
ALTER TABLE `tms_vehicle`
  ...
  ADD UNIQUE KEY `uq_tv_driver_user` (`driver_user_id`),
  ADD KEY `idx_v_category` (`v_category`),
  ADD KEY `idx_tv_driver_user` (`driver_user_id`),
  ADD KEY `idx_vehicle_default_driver` (`default_driver_id`);
...
ALTER TABLE `tms_vehicle_categories`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_tms_vehicle_categories_name` (`name`);

--
-- Indexes for table `vehicles_legacy`
--
ALTER TABLE `vehicles_legacy`
  ADD PRIMARY KEY (`id`),
...
--
ALTER TABLE `tms_user_add_driver`
  MODIFY `d_u_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
...
--
-- AUTO_INCREMENT for table `vehicles_legacy`
--
ALTER TABLE `vehicles_legacy`
...
ALTER TABLE `tms_bookings`
  ...
  ADD CONSTRAINT `fk_tb_vehicle` FOREIGN KEY (`vehicle_id`) REFERENCES `tms_vehicle` (`v_id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
ALTER TABLE `tms_driver_report`
  ...
  ADD CONSTRAINT `fk_report_vehicle` FOREIGN KEY (`vehicle_id`) REFERENCES `tms_vehicle` (`v_id`) ON DELETE SET NULL ON UPDATE CASCADE;
